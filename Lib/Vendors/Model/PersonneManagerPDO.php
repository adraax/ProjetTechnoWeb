...
class PersonneManagerPDO extends PersonneManager
{
	...
	public function getListSansLicence()
	{
		...
		$requete = $this->dao->prepare('SELECT * FROM personne WHERE id NOT IN(
	...
	//Permet d'obtenir la liste des personnes non licenciées qui ne sont pas bénévoles pour une compétition
	public function getListBenevolesDispos($id_competition)
	{
		$personnes = [];
	   
		$requete = $this->dao->prepare('SELECT * FROM personne WHERE id NOT IN(
			SELECT id_personne FROM licence WHERE id_personne IS NOT NULL) AND id NOT IN(
			SELECT id_personne FROM accompagnateur_benevole WHERE id_competition = :id_competition AND id_personne IS NOT NULL)');
